...
export default function QrCodeGeneratorTool({ loggedIn }: { loggedIn?: boolean }) {
  ...
  // Color Controls — default matches Classic Black preset
              ...
              {activeSubTab === 'colors' && (
                <div className="space-y-5 animate-fadeIn">
                  <div>
                    ...
                    {/* 3-column grid — names always fit, no truncation */}
                      ...
                      {COLOR_PRESETS.map((preset, i) => {
                        ...
                        return (
                          ...
                          >
                            {/* Solid Color Circle */}
                            <div
                              className="w-8 h-8 rounded-full border-2 border-white/40 shadow-md flex items-center justify-center shrink-0"
                              style={{ backgroundColor: preset.swatch, boxShadow: '0 1px 4px rgba(0,0,0,0.25)' }}
                            >
                              {isSelected && (
                                <span className="material-symbols-outlined text-white drop-shadow-sm" style={{ fontSize: 14, fontVariationSettings: "'wght' 700" }}>
                                  check
                                </span>
                              )}
                            </div>
                            <span className={`text-[11px] font-bold leading-tight ${isSelected ? 'text-blue-700 dark:text-blue-300' : 'text-slate-700 dark:text-slate-300'}`}>
                              {preset.name}
                            </span>
                          </button>
                        )
